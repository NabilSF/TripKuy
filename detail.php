...
if (!$id_hotel || !is_numeric($id_hotel)) die("Hotel tidak valid!");
...
if (!$hotel) die("Hotel tidak ditemukan!");
...
$kamar_data = mysqli_stmt_get_result($stmt_kamar);

// FIX: Ambil semua data kamar dan simpan ke array
$kamar_data_all = [];
if (mysqli_num_rows($kamar_data) > 0) {
    $kamar_data_all = mysqli_fetch_all($kamar_data, MYSQLI_ASSOC);
    $first_room = $kamar_data_all[0] ?? null;
} else {
    $first_room = null;
    $kamar_data_all = [];
}

$review_query = "SELECT r.*, u.nama FROM review r JOIN users u ON r.id_user = u.id_user LIMIT 3";
$review_result = mysqli_query($conn, $review_query);

$main_image = !empty($hotel['gambar']) ? $hotel['gambar'] : 'https://images.unsplash.com/photo-1566073771259-6a8506099945?auto=format&fit=crop&w=1200&q=80';
...
<head>
    ...
    <title><?= htmlspecialchars($hotel['nama_hotel']) ?> - TripKuy</title>
